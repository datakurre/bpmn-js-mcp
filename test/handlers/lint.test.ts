...
import { handleLintDiagram } from "../../src/handlers/lint";
...
describe("handleLintDiagram", () => {
  ...
  it("returns no start/end event errors for valid start → task → end process", async () => {
    ...
    const taskId = await addElement(diagramId, "bpmn:Task", { name: "Do something", x: 250, y: 100 });
  ...
  it("reports disconnected element", async () => {
    ...
    const hasDisconnected = res.issues.some(
      (i: any) => i.rule === "no-disconnected" || i.rule === "no-implicit-start" || i.rule === "no-implicit-end",
    );
